// ════════════════════════════════════════════════════════════════════════════
// FILE: src/app/b2b-dashboard/page.tsx   ← CREATE FOLDER + FILE (new)
// ════════════════════════════════════════════════════════════════════════════
'use client'
import { useState } from 'react'

const EMPLOYEES = [
  { id: 1, name: 'Hans Mueller',    origin: 'Berlin',       city: 'Athens',    role: 'Senior Developer',   day: 7, scores: [42,55,63,71,78,84,89], risk: 'low',    flag: '🇩🇪', joined: '3 weeks ago' },
  { id: 2, name: 'Sofia Andreou',   origin: 'Thessaloniki', city: 'Amsterdam', role: 'Product Manager',    day: 5, scores: [61,68,72,69,65],          risk: 'medium', flag: '🇬🇷', joined: '2 weeks ago' },
  { id: 3, name: 'Luca Bianchi',    origin: 'Milan',        city: 'Berlin',    role: 'UX Designer',        day: 3, scores: [50,45,40],                 risk: 'high',   flag: '🇮🇹', joined: '1 week ago'  },
  { id: 4, name: 'Marie Dubois',    origin: 'Paris',        city: 'Lisbon',    role: 'Finance Analyst',    day: 6, scores: [55,60,70,77,82,88],         risk: 'low',    flag: '🇫🇷', joined: '3 weeks ago' },
  { id: 5, name: 'Piotr Nowak',     origin: 'Warsaw',       city: 'Madrid',    role: 'Backend Engineer',   day: 4, scores: [35,40,38,42],               risk: 'high',   flag: '🇵🇱', joined: '10 days ago' },
  { id: 6, name: 'Elena Vasquez',   origin: 'Barcelona',    city: 'Stockholm', role: 'HR Specialist',      day: 7, scores: [70,74,79,83,86,90,93],      risk: 'low',    flag: '🇪🇸', joined: '4 weeks ago' },
]
...
export default function B2BDashboard() {
  const [tab, setTab] = useState<'overview' | 'employees' | 'alerts'>('overview')
  const [selectedId, setSelectedId] = useState<number | null>(null)

  const atRisk = EMPLOYEES.filter(e => e.risk === 'high')
  const avgScore = Math.round(EMPLOYEES.reduce((s, e) => s + e.scores[e.scores.length - 1], 0) / EMPLOYEES.length)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070707', color: '#fff', fontFamily: '"DM Sans", -apple-system, sans-serif', display: 'flex' }}>
      ...
      {/* ── Sidebar ──────────────────────────────────────────────────── */}
      <aside style={{ width: '210px', borderRight: '1px solid #0f0f0f', padding: '24px 14px', display: 'flex', flexDirection: 'column', background: '#050505', flexShrink: 0 }}>
        ...
        {([
          ...
          <button key={t.id} onClick={() => { setTab(t.id); setSelectedId(null) }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: tab === t.id ? '#0f2a1a' : 'none', border: tab === t.id ? '1px solid #1a3a1f' : '1px solid transparent', color: tab === t.id ? '#4ade80' : '#333', fontSize: '14px', fontWeight: '600', cursor: 'pointer', width: '100%', textAlign: 'left', marginBottom: '4px', fontFamily: 'inherit' }}>
            <span>{t.icon}</span>
            {t.label}
        ...
        <div style={{ marginTop: 'auto', padding: '12px', background: '#0e0e0e', borderRadius: '12px', border: '1px solid #111' }}>
          ...
          <p style={{ color: '#fff', fontWeight: '700', fontSize: '13px' }}>Acme Corp</p>
        ...
        <div style={{ borderBottom: '1px solid #0d0d0d', padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#070707', position: 'sticky', top: 0, zIndex: 10 }}>
          <div>
            <h1 style={{ fontSize: '17px', fontWeight: '800' }}>
              { tab === 'overview' ? 'Retention Overview' : tab === 'employees' ? 'All Employees' : '⚠ At-Risk Alerts' }
            </h1>
            <p style={{ color: '#2a2a2a', fontSize: '11px', marginTop: '2px', fontFamily: 'DM Mono, monospace' }}>Live · Updated in real-time</p>
          </div>
          {atRisk.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#2a0505', border: '1px solid #3a1010', borderRadius: '10px', padding: '8px 14px' }}>
              ...
              <span style={{ color: '#f87171', fontSize: '13px', fontWeight: '700' }}>{atRisk.length} employees need attention</span>
            </div>
          )}
        </div>
            ...
            <div style={{ animation: 'fadeIn 0.3s ease both' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '24px' }}>
                {[
                  ...
                  { l: 'Bootcamp Completed',  v: `${completed}/${EMPLOYEES.length}`, c: '#8b5cf6' },
              ...
              <div style={{ background: '#0a140a', border: '1px solid #1a3a1f', borderRadius: '16px', padding: '22px 26px', marginBottom: '24px' }}>
                <p style={{ color: '#4ade80', fontSize: '11px', fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '14px', fontFamily: 'DM Mono, monospace' }}>ROI CALCULATOR — why prevention beats rehiring</p>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                  {[
                    { l: 'Cost to rehire 1 developer',      v: '€15,000', c: '#f87171' },
                    { l: 'Annual Glotto cost per employee',  v: '€1,200',  c: '#fbbf24' },
                    { l: 'Net saving (1 prevented churn)',   v: '€13,800', c: '#4ade80' },
                  ].map(({ l, v, c }) => (
                    <div key={l}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {EMPLOYEES.map(e => {
                  ...
                  return (
                    <div key={e.id} className="r" onClick={() => { setSelectedId(e.id); setTab('employees') }} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 2fr 1fr', gap: '16px', alignItems: 'center', padding: '12px 16px', background: '#0a0a0a', border: `1px solid ${e.risk === 'high' ? '#2a1010' : '#0f0f0f'}`, borderRadius: '12px', transition: 'background 0.15s' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              {selected ? (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div style={{ background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '28px' }}>
                      ...
                      {[
                        ...
                        { k: 'Latest',     v: `${selected.scores[selected.scores.length - 1]}/100` },
                      ].map(({ k, v }) => (
                    ...
                    <div style={{ background: '#0e0e0e', border: `1px solid ${RISK[selected.risk].border}`, borderRadius: '16px', padding: '28px' }}>
                      ...
                      <div style={{ background: RISK[selected.risk].bg, border: `1px solid ${RISK[selected.risk].border}`, borderRadius: '10px', padding: '12px 14px' }}>
                        ...
                        <p style={{ color: '#555', fontSize: '12px', lineHeight: 1.5 }}>
                          {selected.risk === 'high' ? 'Declining 3+ sessions. Recommend HR check-in within 48 hours.' : selected.risk === 'medium' ? 'Scores plateauing. Monitor next 2 sessions.' : 'Steady progress. On track for full integration.'}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div style={{ background: '#0e0e0e', border: '1px solid #111', borderRadius: '16px', overflow: 'hidden' }}>
                  ...
                  {EMPLOYEES.map(e => {
                    const score = e.scores[e.scores.length - 1]
                    return (
                      <div key={e.id} className="r" onClick={() => setSelectedId(e.id)} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr 1fr 80px', padding: '14px 20px', borderBottom: '1px solid #0a0a0a', alignItems: 'center', background: e.risk === 'high' ? '#0f0505' : 'transparent', transition: 'background 0.15s' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <span style={{ fontSize: '18px' }}>{e.flag}</span>
                          <div>
                            <p style={{ color: '#fff', fontWeight: '700', fontSize: '13px' }}>{e.name}</p>
                    ...
                    <div key={e.id} style={{ background: '#0f0505', border: '1px solid #3a1010', borderRadius: '16px', padding: '24px', display: 'flex', gap: '18px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ color: '#555', fontSize: '13px', lineHeight: 1.6, marginBottom: '16px' }}>
                          Confidence has dropped for 3 consecutive sessions ({e.scores.slice(-3).join(' → ')}). Pattern correlates with culture-shock disengagement. Action recommended within 48 hours.
                        </p>
