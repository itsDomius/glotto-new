...
// ── Demo messages ────────────────────────────────────────────────────────
const DEMO_MESSAGES = [
  { role: 'lex', text: "Day 3 mission: You're at the Athens tax office. You need an AFM number to sign your rental contract. The clerk only speaks Greek. Go." },
  { role: 'user', text: 'Γεια σας. Θέλω να κάνω αίτηση για AFM παρακαλώ... I need the tax number for my contract?' },
  { role: 'lex', text: "Solid start — mixing languages is exactly what real expats do. The clerk replies: 'Έχετε διαβατήριο και αποδεικτικό κατοικίας;' — they're asking for your passport and proof of address. Do you have them?" },
  { role: 'user', text: 'Ναι, έχω το διαβατήριό μου. Για την κατοικία — έχω τη σύμβαση ενοικίασης. Is that enough?' },
  { role: 'lex', text: "Perfect process. Correct documents, correct order. The clerk says: 'Εντάξει. Θα σας δώσω αριθμό σειράς.' — you're getting a ticket number. Mission passed. +50 XP. You just navigated Greek bureaucracy. Real expats can't do this on day 3." },
]
...
function TypewriterMessage({ text, onDone }: { text: string; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (i >= text.length) { clearInterval(interval); setTimeout(onDone, 600) }
    }, 16)
...
function LiveDemo() {
  ...
  useEffect(() => {
    if (inView && !running) { setRunning(true); setMessages([DEMO_MESSAGES[0]]); setMsgIndex(0) }
  }, [inView])
  const handleDone = () => {
    const next = msgIndex + 1
    if (next < DEMO_MESSAGES.length) {
      setTimeout(() => { setMessages(prev => [...prev, DEMO_MESSAGES[next]]); setMsgIndex(next) }, 400)
    }
  }
  return (
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', justifyContent: isLex ? 'flex-start' : 'flex-end', gap: '10px', alignItems: 'flex-start' }}>
              {isLex && (
                <div style={{ width: 32, height: 32, borderRadius: '10px', background: '#111', border: '1px solid #1f1f1f', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Star size={14} color="#4ade80" />
                </div>
              )}
              <div style={{ background: isLex ? '#111' : '#0f2a1a', border: `1px solid ${isLex ? '#1a1a1a' : '#1a3a1f'}`, borderRadius: isLex ? '4px 16px 16px 16px' : '16px 4px 16px 16px', padding: '12px 16px', maxWidth: '82%', fontSize: '14px', lineHeight: 1.65, color: isLex ? '#ccc' : '#fff' }}>
                {isLast && running ? <TypewriterMessage text={msg.text} onDone={handleDone} /> : msg.text}
              </div>
            </div>
          )
        })}
...
export default function LandingPage() {
  ...
  const expats  = useCounter(281,   1600, statsRef.inView)
  const churn   = useCounter(38,    1400, statsRef.inView)
  const rehire  = useCounter(15000, 2200, statsRef.inView)
  ...
  // ── Icon map for "How it works" and "Features" cards ─────────────────
  const HOW_IT_WORKS = [
    { num: '01', Icon: MapPin,    iconColor: '#4ade80', title: 'Tell us your city',    desc: 'Pick where you landed. Berlin, Athens, Lisbon — Glotto adapts to your exact local bureaucracy, language, and survival priorities.' },
    { num: '02', Icon: Drama,     iconColor: '#60a5fa', title: 'Survive daily missions', desc: "Each mission is a real scenario: get your tax ID, open a bank account, negotiate your lease. You talk to an AI NPC. Fail the process — even with perfect language — and you do it again." },
    { num: '03', Icon: Trophy,    iconColor: '#fbbf24', title: 'Unlock real rewards',  desc: 'Pass the bank mission → get a real N26 offer. Pass the SIM mission → get Airalo. Our affiliate partners turn your success into money back in your pocket.' },
  ]

  const FEATURES = [
    { Icon: Drama,     accent: '#4ade80', title: 'AI NPC Simulations',       desc: "7 survival missions per city. Graded on bureaucratic process (60%) AND language (40%). Perfect Greek but wrong documents? You fail. That's by design." },
    { Icon: Siren,     accent: '#f87171', title: 'Emergency Panic Button',    desc: 'Every page has a red FAB. Click it, select your location (Pharmacy, Police, Bank, Hospital) and get an instant survival script with phonetic pronunciation.' },
    { Icon: Zap,       accent: '#c084fc', title: 'Commitment Mode',           desc: 'Stake €30 in escrow. Complete 5 missions in 7 days — get it all back. Fail — we keep it. Stakers complete 90% of missions vs 23% standard.' },
    { Icon: Building2, accent: '#fbbf24', title: 'Affiliate Rewards',         desc: 'Pass the bank mission → N26 offer. Pass the SIM mission → Airalo eSIM. Pass health → SafetyWing. Negative CAC model.' },
    { Icon: BarChart3, accent: '#60a5fa', title: 'Confidence Tracker',        desc: 'Every session scored. Process score vs language score visualised over time. You can see exactly where you are in your integration journey.' },
    { Icon: Building2, accent: '#fb923c', title: 'HR Shield for Companies',   desc: 'B2B dashboard for HR directors. Employee table with confidence sparklines, churn risk flags, and ROI calculator. €15k to rehire vs €1.2k Glotto.' },
  ]
  ...
  return (
    <div style={{ background: '#0a0a0a', color: '#fff', fontFamily: '"DM Sans", -apple-system, BlinkMacSystemFont, sans-serif', overflowX: 'hidden' }}>
      ...
      {/* Nav */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '0 40px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: scrolled ? 'rgba(10,10,10,0.9)' : 'transparent', backdropFilter: scrolled ? 'blur(16px)' : 'none', borderBottom: scrolled ? '1px solid #111' : '1px solid transparent', transition: 'all 0.3s' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button className="cta-btn" onClick={() => router.push('/auth/signup')} style={{ background: '#4ade80', border: 'none', borderRadius: '10px', color: '#050f06', fontSize: '14px', fontWeight: '700', padding: '9px 20px', cursor: 'pointer', fontFamily: 'inherit' }}>Start free →</button>
        </div>
      </nav>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '120px 24px 80px', textAlign: 'center', position: 'relative' }}>
        ...
        <p className="hero-sub" style={{ color: '#666', fontSize: 'clamp(16px, 2vw, 20px)', maxWidth: '540px', lineHeight: 1.7, marginBottom: '44px' }}>
          The AI-driven Relocation OS that guides you through local bureaucracy, housing, and daily friction — not grammar drills, not streaks.<br />
        ...
        <div className="hero-cta" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center' }}>
          <button className="cta-btn" onClick={() => router.push('/auth/signup')} style={{ background: '#4ade80', border: 'none', borderRadius: '14px', color: '#050f06', fontSize: '16px', fontWeight: '800', padding: '16px 32px', cursor: 'pointer', fontFamily: 'inherit' }}>Try a Survival Mission →</button>
          <InstallButton />
          <button onClick={() => router.push('/pricing')} style={{ background: 'transparent', border: '1px solid #222', borderRadius: '14px', color: '#888', fontSize: '16px', padding: '16px 28px', cursor: 'pointer', fontFamily: 'inherit' }}>See pricing</button>
        </div>
        <p style={{ color: '#333', fontSize: '13px', marginTop: '16px', animation: 'fadeIn 0.5s ease 0.9s both', opacity: 0 }}>No credit card required · Try 2 Missions free</p>
      ...
      <section ref={statsRef.ref} style={{ padding: '80px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto', textAlign: 'center' }}>
          <p style={{ color: '#333', fontSize: '13px', fontWeight: '600', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '60px' }}>The relocation industry is failing expats</p>
        ...
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '72px' }}>
            ...
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(36px, 5vw, 56px)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
              From airport to integrated<br /><span style={{ color: '#4ade80', fontStyle: 'italic' }}>in 7 days.</span>
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
            {HOW_IT_WORKS.map(({ num, Icon, iconColor, title, desc }, i) => (
              <div key={i} className="card" style={{ background: '#0e0e0e', border: '1px solid #181818', borderRadius: '20px', padding: '32px 28px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                  <div style={{ width: 44, height: 44, borderRadius: '12px', background: `${iconColor}12`, border: `1px solid ${iconColor}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ maxWidth: '720px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '56px' }}>
            ...
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(32px, 4vw, 48px)', lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: '16px' }}>
              This is what Mission 3<br /><span style={{ color: '#4ade80', fontStyle: 'italic' }}>actually looks like.</span>
            </h2>
      ...
      <section style={{ padding: '100px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '72px' }}>
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(36px, 5vw, 56px)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
              Built for survival.<br /><span style={{ color: '#4ade80', fontStyle: 'italic' }}>Not vocabulary.</span>
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
            {FEATURES.map(({ Icon, accent, title, desc }, i) => (
              <div key={i} className="card" style={{ background: '#0e0e0e', border: '1px solid #181818', borderRadius: '20px', padding: '28px', display: 'flex', gap: '20px' }}>
                <div style={{ width: 44, height: 44, borderRadius: '12px', background: `${accent}12`, border: `1px solid ${accent}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ maxWidth: '840px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(36px, 5vw, 52px)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
              The current &quot;solution&quot;<br /><span style={{ color: '#f87171', fontStyle: 'italic' }}>is a Wikipedia article.</span>
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {[
              { label: 'How expats relocate today', items: ['Google "how to get AFM in Greece"', 'Read a 3-year-old Reddit thread', 'Show up with the wrong documents', 'Come back 4 times to the same office', 'Quit and move home within 12 months'], isGlotto: false },
              { label: 'With Glotto', items: ['Simulate the exact office conversation before you go', 'Know exactly which documents to bring', 'Practice the right bureaucratic phrases', 'Pass missions = build real integration confidence', 'Get rewarded by our partners when you succeed'], isGlotto: true },
            ].map(({ label, items, isGlotto }) => (
              <div key={label} style={{ background: isGlotto ? '#0a140a' : '#0e0a0a', border: `1px solid ${isGlotto ? '#1a3a1a' : '#2a1010'}`, borderRadius: '20px', padding: '32px' }}>
                  ...
                  {items.map((item, i) => (
                    <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                      {isGlotto
                        ...
                        : <X size={14} color="#ff4444" style={{ flexShrink: 0, marginTop: '2px' }} />
                      }
                      <span style={{ color: isGlotto ? '#bbb' : '#555', fontSize: '14px', lineHeight: 1.5 }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section style={{ padding: '60px 24px', borderTop: '1px solid #111' }}>
        <div style={{ maxWidth: '840px', margin: '0 auto' }}>
          <div style={{ background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: '20px', padding: '36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px', flexWrap: 'wrap' }}>
            ...
            <button className="cta-btn" onClick={() => router.push('/b2b-dashboard')} style={{ background: '#fbbf2415', border: '1px solid #fbbf2440', color: '#fbbf24', borderRadius: '14px', padding: '14px 24px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
      ...
      <section style={{ padding: '120px 24px', borderTop: '1px solid #111', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ color: '#333', fontSize: '13px', fontWeight: '600', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '24px' }}>281 million expats. One OS.</p>
        <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(44px, 7vw, 80px)', lineHeight: 1.0, letterSpacing: '-0.03em', marginBottom: '28px' }}>
          Your city.<br /><span style={{ color: '#4ade80', fontStyle: 'italic' }}>Your survival.</span>
        </h2>
        <p style={{ color: '#555', fontSize: '18px', maxWidth: '420px', margin: '0 auto 44px', lineHeight: 1.6 }}>The bureaucracy you've been dreading starts with one simulation.</p>
        <button className="cta-btn" onClick={() => router.push('/auth/signup')} style={{ background: '#4ade80', border: 'none', borderRadius: '16px', color: '#050f06', fontSize: '18px', fontWeight: '800', padding: '20px 44px', cursor: 'pointer', fontFamily: 'inherit', display: 'inline-block' }}>
          Start your first mission free →
        </button>
        <p style={{ color: '#2a2a2a', fontSize: '13px', marginTop: '16px' }}>No credit card · 2 missions free · Cancel anytime</p>
      </section>
